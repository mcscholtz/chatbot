...
import chatbotcore.data_utils as data_utils
import chatbotcore.seq2seq_model as seq2seq_model
...
def create_model(session, forward_only, config):
  ...
  model = seq2seq_model.Seq2SeqModel( config['enc_vocab_size'], config['dec_vocab_size'], _buckets, config['layer_size'], config['num_layers'], config['max_gradient_norm'], config['batch_size'], config['learning_rate'], config['learning_rate_decay_factor'], forward_only=forward_only)

  if 'pretrained_model' in config:
      model.saver.restore(session,config['pretrained_model'])
      return model

  ckpt = tf.train.get_checkpoint_state(os.path.join('chatbotcore',config['working_directory']))
...
def decode(query_q, resp_q, ctl_q):
  with tf.Session() as sess:
    
    #load configs
    config = get_config('chatbotcore/seq2seq.ini')
    ...
    # Load vocabularies.
    print (config)
    enc_vocab_path = os.path.join(config['working_directory'],"vocab%d.from" % config['enc_vocab_size'])
    dec_vocab_path = os.path.join(config['working_directory'],"vocab%d.to" % config['dec_vocab_size'])

    enc_vocab, _ = data_utils.initialize_vocabulary(os.path.join('chatbotcore', enc_vocab_path))
    _, rev_dec_vocab = data_utils.initialize_vocabulary(os.path.join('chatbotcore', dec_vocab_path))
    ...
    status = ctl_q.get()
    while status == 'START':
      print("Bot waiting for data....")
      sentence = query_q.get()
      print('Bot analzying....')
      ...
      resp = " ".join([tf.compat.as_str(rev_dec_vocab[output]) for output in outputs])
      resp_q.put(resp)

def beam_search(data):
    for logit in data:
        #print(logit)
        print(int(np.argmax(logit, axis=1)))
        vals = np.argpartition(logit, -3, axis=1)[-3:]
        print(vals)
...
def init_session(sess, conf='seq2seq.ini'):
    ...
    # Load vocabularies.
    enc_vocab_path = os.path.join(gConfig['working_directory'],"vocab%d.from" % gConfig['enc_vocab_size'])
    dec_vocab_path = os.path.join(gConfig['working_directory'],"vocab%d.to" % gConfig['dec_vocab_size'])
